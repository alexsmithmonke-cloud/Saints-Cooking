...
net.Receive("SendRecipeInUse", function() --Starting the cookin' process
    ...
    local AbleToTakeBackFood = net.ReadBool()
    local CookingTimeStart = net.ReadFloat()
    local AmountOfTimePanHasBeenCooked = net.ReadFloat()
    if stove then
        stove.recipeData = recipe or {}
        stove.AbleToTakeBackFood = AbleToTakeBackFood
        stove.Cooking = IsCooking
        stove.CookingTimeStart = CookingTimeStart
        stove.AmountOfTimePanHasBeenCooked = AmountOfTimePanHasBeenCooked
        stove.EmptyPan = EmptyPan
    end
end)

net.Receive("EmptyPanDetected", function()
    local EmptyPanDetected = net.ReadBool()
    local stove = net.ReadEntity()
    if stove then
        stove.EmptyPanDetected = EmptyPanDetected
    end
end)
...
function ENT:DrawPanModel(Exist) --to make the prop of the pan
    if Exist and self:IsValid() then
...
function ENT:Draw()
    ...
    local AmountOfTimePanHasBeenCooked = self.AmountOfTimePanHasBeenCooked or 0
    local AbleToTakeBackFood = self.AbleToTakeBackFood
    local EmptyPanDetected = self.EmptyPanDetected
    ...
    cam.Start3D2D(pos,ang1, 0.1)
        ...
        if panInside then
            ...
            if not beingcooked and AbleToTakeBackFood or EmptyPanDetected then
                ...
                textpos = -220
                draw.WordBox(6,100,textpos,"Cooking.. "..AmountOfTimePanHasBeenCooked,"NormalText",Color(92,88,88,178),Color(190,187,187),TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER) --Using this to state how much time has passed
                if AmountOfTimePanHasBeenCooked >= 3  then
