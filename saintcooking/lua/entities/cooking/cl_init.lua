...
function ENT:DrawPanModel(Exist) --tomake the prop of the pan
    if Exist then
...
function ENT:Draw()
    ...
    cam.Start3D2D(pos1, ang1, 0.1)
        ...
        draw.DrawText("Turn "..self:GetStoveState(), "NormalText", -1, 40, Color(0, 0, 0), TEXT_ALIGN_CENTER)
    ...
    cam.Start3D2D(pos,ang1, 0.1)
        local textpos = (#contents * -40) - 200
        if panInside then
            draw.WordBox(4,100, textpos - 40,"Pan","NormalText",Color(92,88,88,178),Color(190,187,187),TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
            if not beingcooked then
                ...
                draw.WordBox(6,100,textpos,"Cooking.. "..displaytime,"NormalText",Color(92,88,88,178),Color(190,187,187),TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
                draw.WordBox(6,100,textpos + 40,recipebeingused.name,"NormalText",Color(92,88,88,178),Color(190,187,187),TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
            end
        end
...
hook.Add("KeyPress", "Stove", function(ply, key)
    ...
    if IsValid(target) and target:GetClass() == "cooking" and ply:GetPos():DistToSqr(target:GetPos()) < 10000 then 
